...
Following strict TDD prevents most testing anti-patterns. Key pitfalls to avoid:

- **Testing mock behavior instead of real behavior** - If you're asserting on mock elements or test IDs, you're verifying the mock works, not your code. Test real components or don't mock them.
- **Adding test-only methods to production classes** - Methods only called in test files pollute production code. Move cleanup and test helpers to test utilities instead.
