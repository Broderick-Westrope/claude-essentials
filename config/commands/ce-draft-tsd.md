...
Use `Skill(writer)` with The Architect persona to:
...
Detect architecture topics from keywords and load relevant sections from `Skill(architecting-systems)`:
...
- `Skill(drafting-tsds)` for structure and template
- `Skill(writer)` with The Architect persona for tone and style
