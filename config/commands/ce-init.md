...
All generated files (rules, CLAUDE.md, skills) must follow the principles from `configuring-claude`. Key points:

- Rules reference skills rather than duplicating skill content
...
Use `configuring-claude` principles (and its references) as the baseline for what "good" looks like. Check for:

**Missing skill references in rules:**
- Rules should reference skills for detailed guidance
- Example: testing.md should mention `writing-tests`
...
Good:
  ...
  + Rules properly reference skills

Suggestions:
  - rules/testing.md: Add reference to writing-tests skill
...
Progressive disclosure issues:
  ...
  - rules/frontend/components.md: Duplicates design skill content
...
When writing tests, load the writing-tests skill for general patterns.

## Flaky Tests

When fixing flaky tests, load the fixing-flaky-tests skill.
...
When designing error handling, load the handling-errors skill.
...
When investigating bugs or unexpected behavior, load the systematic-debugging skill.
...
Before claiming work is complete, load the verification-before-completion skill.
...
When designing APIs, load the architecting-systems skill for general patterns.
...
- Existing codebase has conventions that differ from skill defaults
- Team has documented patterns that should be enforced

**When NOT to generate:**
- Simple projects where skills cover all patterns
- Greenfield projects without established conventions
- If it would duplicate skill content
