...
For diagram guidance, use `Skill(visualizing-with-mermaid)`.
...
Use `Skill(writer)` with **The Architect** persona for:
...
- `Skill(architecting-systems)` - Architecture principles for informed option generation
- `Skill(writer)` with **The Architect** persona - Tone and structure for technical docs
- `Skill(documenting-systems)` - General documentation best practices
- `Skill(visualizing-with-mermaid)` - Creating architecture and flow diagrams
- `Skill(systematic-debugging)` - Root cause analysis when designing fixes
- `Skill(strategy-writer)` - For strategic business documents (not technical specs)
